                        ...
                        unsigned char *output)
{
    ...
    mbedtls_mpi T, T_lo, T_hi;

	mbedtls_mpi R_INV_P, M_P_P, R_SQR_P, R_INV_Q, M_P_Q, R_SQR_Q;
    ...
    /* MPI Initialization */
	mbedtls_mpi_init( &T );
	mbedtls_mpi_init( &T_lo );
	mbedtls_mpi_init( &T_hi );

    mbedtls_mpi_init( &R_INV_P );
    mbedtls_mpi_init( &M_P_P );
    mbedtls_mpi_init( &R_SQR_P );
    mbedtls_mpi_init( &R_INV_Q );
    mbedtls_mpi_init( &M_P_Q );
    mbedtls_mpi_init( &R_SQR_Q );	

	MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &R_INV_P, ctx->P.n ) );
	MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &M_P_P, ctx->P.n ) );
	MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &R_SQR_P, ctx->P.n ) );
	MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &R_INV_Q, ctx->Q.n ) );
	MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &M_P_Q, ctx->Q.n ) );
	MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &R_SQR_Q, ctx->Q.n ) );		

	MBEDTLS_MPI_CHK( pka_precompute(&g_pka_ctxt, (uint8_t *)R_INV_P.p, (uint8_t *)M_P_P.p, (uint8_t *)R_SQR_P.p, 
		(const uint8_t *)ctx->P.p, ctx->len / 2) );

	MBEDTLS_MPI_CHK( pka_precompute(&g_pka_ctxt, (uint8_t *)R_INV_Q.p, (uint8_t *)M_P_Q.p, (uint8_t *)R_SQR_Q.p, 
		(const uint8_t *)ctx->Q.p, ctx->len / 2) );
	...
	MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &T_hi, input, ctx->len / 2 ) );
	MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &T_lo, &input[ctx->len / 2], ctx->len / 2) );
	
	pka_crt_modexp_params_t crt_params = 
	{
		.msg_lo = (const uint8_t *)T_lo.p,
		.msg_hi = (const uint8_t *)T_hi.p,
		.p = (const uint8_t *)ctx->P.p,
		.q = (const uint8_t *)ctx->Q.p,
		.u = (const uint8_t *)ctx->QP.p,
		.d_p = (const uint8_t *)ctx->DP.p,
		.d_q = (const uint8_t *)ctx->DQ.p,
		.r_sqr_mod_p = (const uint8_t *)R_SQR_P.p,
		.p_prime = (const uint8_t *)M_P_P.p,
		.r_sqr_mod_q = (const uint8_t *)R_SQR_Q.p,
		.q_prime = (const uint8_t *)M_P_Q.p,
		.size = ctx->len / 2,
		.enable_timing_attack_hardening = 1
	};
	MBEDTLS_MPI_CHK( pka_crt_modexp(&g_pka_ctxt, &crt_params, (uint8_t*)T.p, 
									((uint8_t*)T.p + ctx->len / 2)) );
...
cleanup:
	...
	mbedtls_mpi_free( &T );
	mbedtls_mpi_free( &T_lo );
	mbedtls_mpi_free( &T_hi );

    mbedtls_mpi_free( &R_INV_P );
    mbedtls_mpi_free( &M_P_P );
    mbedtls_mpi_free( &R_SQR_P );
    mbedtls_mpi_free( &R_INV_Q );
    mbedtls_mpi_free( &M_P_Q );
    mbedtls_mpi_free( &R_SQR_Q );	
