                           ...
                           void *p_rng)
{
     ...
     */
    limbs = (mbedtls_mpi_size(&ctx->P) + sizeof(mbedtls_mpi_uint) - 1 ) / sizeof(mbedtls_mpi_uint);
	MBEDTLS_MPI_CHK(mbedtls_mpi_shrink( &ctx->G, limbs ));
	MBEDTLS_MPI_CHK(mbedtls_mpi_shrink( &ctx->X, limbs ));
	MBEDTLS_MPI_CHK(mbedtls_mpi_shrink( &ctx->P, limbs ));
                            ...
                            void *p_rng)
{
	...
	MBEDTLS_MPI_CHK(mbedtls_mpi_shrink( &ctx->GY, limbs ));
	MBEDTLS_MPI_CHK(mbedtls_mpi_shrink( &ctx->X, limbs ));
	MBEDTLS_MPI_CHK(mbedtls_mpi_shrink( &ctx->P, limbs ));	
    /* Do modular exponentiation */
    MBEDTLS_MPI_CHK(mbedtls_mpi_exp_mod_hw(&ctx->K, &ctx->GY, &ctx->X,
                                        &ctx->P, &ctx->RP));
...
int mbedtls_dhm_pka_self_test(int verbose)
{
    ...
    if( ( ret = mbedtls_dhm_parse_dhm( &dhm1,
    ...
    if( ( ret = mbedtls_dhm_parse_dhm( &dhm2,
                    (const unsigned char *) mbedtls_test_dhm_params,
                    mbedtls_test_dhm_params_len ) ) != 0 )
    {
        if( verbose != 0 )
	...
	dhm_len1 = mbedtls_mpi_size(&dhm1.P);
	dhm_len2 = mbedtls_mpi_size(&dhm2.P);
    if( verbose != 0 )
        mbedtls_printf( "  Generate private value & Calculate public value: " );

	buf1 = mbedtls_calloc(dhm_len1, 1);
	if (!buf1)
	{
		ret = 1;
		goto exit;
	}
	buf2 = mbedtls_calloc(dhm_len2, 1);
	if (!buf2)
	{
		ret = 1;
		goto exit;
	...
	if( ( ret = mbedtls_dhm_make_public( &dhm1, (int) dhm_len1, buf1, dhm_len1,
	...
	if( ( ret = mbedtls_dhm_make_public( &dhm2, (int) dhm_len2, buf2, dhm_len2,
								 mbedtls_ctr_drbg_random, &ctr_drbg ) ) != 0 )
	{
		if( verbose != 0 )
			mbedtls_printf( " failed\n	! mbedtls_dhm_make_public returned %d\n\n", ret );
	...
	if( ( ret = mbedtls_dhm_read_public(&dhm1, (const unsigned char *)buf2, dhm_len2)))
	{
		if( verbose != 0 )
	...
	if( ( ret = mbedtls_dhm_read_public(&dhm2, (const unsigned char *)buf1, dhm_len1)))
	{
		if( verbose != 0 )
			mbedtls_printf( " failed\n	! mbedtls_dhm_read_public returned %d\n\n", ret );
	...
	/* calculate secret */
    if( verbose != 0 )
        mbedtls_printf( "  Calculate shared secret: " );

	if( ( ret = mbedtls_dhm_calc_secret(&dhm1, buf1, dhm_len1, &olen, 0, 0) != 0 ))
	{
		if( verbose != 0 )
	...
	if( ( ret = mbedtls_dhm_calc_secret(&dhm2, buf2, dhm_len2, &olen, 0, 0) != 0 ))
	{
		if( verbose != 0 )
			mbedtls_printf( " failed\n	! mbedtls_dhm_calc_secret returned %d\n\n", ret );
	...
	if (ret = memcmp(buf1, buf2, olen) != 0)
	{
		if( verbose != 0 )
	...
	if( verbose != 0 )
        mbedtls_printf( "  passed\n" );
exit:
    mbedtls_dhm_free( &dhm1 );
	mbedtls_dhm_free( &dhm2 );
